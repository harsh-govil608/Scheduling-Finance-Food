                    ...
                    items(pendingIOwe, key = { "owe-${it.id}" }) { participant ->
                        Card(
                            ...
                            Column(Modifier.padding(16.dp)) {
                                Text(
                                    "To ${participant.payerName.ifBlank { "Someone" }}" +
                                        participant.description.takeIf { it.isNotBlank() }?.let { " - $it" }.orEmpty(),
                                    style = MaterialTheme.typography.bodyLarge
                                )
                                Spacer(Modifier.height(4.dp))
                                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                                    Text("₹${"%.2f".format(participant.shareAmount)}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                                    Text(
